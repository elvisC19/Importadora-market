    ...
    <div className="flex min-h-screen bg-slate-50 text-slate-800 pt-16 overflow-x-hidden">
      ...
      <aside className="hidden lg:flex flex-col w-64 bg-slate-900 text-white fixed top-16 bottom-0 left-0 p-6 border-r border-slate-800 z-30 select-none">
        ...
        <nav className="flex-1 space-y-2">
          ...
          <Link to="/importadora/pedidos" className="flex items-center gap-3 px-4 py-3 bg-amber-600 text-white rounded-lg shadow-sm font-bold">
      ...
      <main className="flex-grow lg:ml-64 p-4 md:p-8 relative min-w-0 space-y-6 text-left">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Gestión de Pedidos Recibidos</h1>
          <p className="text-sm text-slate-500">Administra las solicitudes de compra de tus clientes, coordina pagos por WhatsApp e imprime notas de entrega.</p>
        </div>

        {/* Sync / Action buttons & Stats */}
        <div className="flex flex-wrap items-center gap-3 self-start md:self-auto no-print">
          <Link
        ...
        {error && (
          <div className="bg-red-50 text-red-700 p-4 rounded-xl border border-red-100 flex items-center gap-3 mb-8 no-print">
        ...
        {/* Tarjeta Blanca Homogénea para la Tabla */}
        <div className="bg-white p-6 rounded-xl border border-slate-100 shadow-sm no-print">
          {/* Filtering Toolbar */}
          ...
          <div className="overflow-x-auto">
            {loading ? (
                      ...
                      >
                        ...
                        <td className="px-4 py-4 text-center">
                          <button
                            ...
                            className="px-3 py-1.5 bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white rounded-lg text-sm font-medium transition-all duration-200 flex items-center gap-1.5 cursor-pointer shadow-sm mx-auto"
                          >
              ...
              <div className="text-center py-20 bg-white">
                ...
                <h3 className="text-xl font-bold text-slate-800 mb-2">Sin ventas aún</h3>
        ...
        {/* AQUÍ VAN LOS DETALLES DEL PEDIDO SELECCIONADO, EL BOTÓN DE WHATSAPP EMPRESARIAL Y EL FORMATO DE IMPRESIÓN */}
        {expandedOrderId && (() => {
          const order = orders.find(o => o.order_id === expandedOrderId);
          ...
          return (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
              ...
              <div id={`print-invoice-${order.order_id}`} className="bg-white p-6 rounded-xl border border-slate-100 shadow-sm flex flex-col text-left">
                ...
                {/* Items list */}
                <div className="border border-slate-100 rounded-xl overflow-hidden shadow-sm mb-6">
                  <table className="w-full text-left border-collapse text-xs">
                     ...
                     <tfoot>
                       <tr className="bg-slate-50 border-t border-slate-200">
                         <td colSpan={3} className="px-4 py-3 text-right font-semibold text-slate-700 uppercase tracking-wider">
